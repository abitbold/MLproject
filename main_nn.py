...
"""
@author: Arjun Alagappan, David Abitbol, Shan Shan Liu
"""
...
Xcv1 = X.loc[((X.index>pd.to_datetime(d1)) & (X.index<=pd.to_datetime(d2))), C].copy()
Ycv1 = X.loc[((X.index>pd.to_datetime(d1)) & (X.index<=pd.to_datetime(d2))), 'Y'].copy()
Xtrain1 = X.loc[((X.index>pd.to_datetime(d0)) & (X.index<=pd.to_datetime(d1))), C].copy()
Ytrain1 = X.loc[((X.index>pd.to_datetime(d0)) & (X.index<=pd.to_datetime(d1))), 'Y'].copy()
...
if cv_nlayers:
    ...
    for nlayers in range(8, 0, -1):
        ...
        ls.append(1)
        nnt, err = npy.nnTrain(Xcv1, Ycv1, ls, drp, drpProb, batch, opt, learning_rate, n_epochs) 
...
if cv_nneurones:
    train_loss = []
    nl = []
    for nneurones in [20,15, 10, 1]: #range(20, 0, -5):
        print(nneurones)
        ls = [nneurones]*nlayers
        ls = np.insert(ls, 0, Xcv1.shape[1]).tolist()
        ls.append(1)
